...
const FILETYPES = ["image/jpg", "image/jpeg", "image/png", "image/webp"]; // Supported file types
...
const downloadByID = async (type, id) => {
  // Function to download a file by its ID - AKA the name of the file
  try {
...
const upload = (file, type, id) => {
  ...
  fs.open(destination, "r+", (err, fd) => {
    ...
    try {
      if (err) {
        if (err.code === "EEXIST") {
          // If the directory already exists, write the file to it
          sharp(file.data).toFile(`${destination}/${setID()}.webp`); // Convert the file to webp and write it to the directory
        } else if (err.code === "ENOENT") {
          // If the directory does not exist, create it and write the file to it
          fs.mkdir(destination, { recursive: true }, (err) => {
            ...
            } // Convert the file to webp and write it to the directory
          });
        } else {
          throw err;
        }
...
const updateFile = (file, type, id) => {
  // Function to update a file in a directory
  if (MAX_FILESIZE < file.size)
    // Check if the file size is greater than the max file size
    throw {
      code: 413,
    ...
    try {
      if (err) {
        ...
        fs.readdir(destination, { recursive: true }, (err, files) => {
          // If the directory exists, delete all the files in it and write the new file to it
          if (err) throw err;
          files.forEach((file) =>
